...
export default class SettingsView {
  constructor(context: vscode.ExtensionContext) {
    ...
    vscode.commands.registerCommand("settingsView.reveal", async () => {
    ...
    vscode.commands.registerCommand("settingsView.changeTitle", async () => {
...
function getTreeItem(key: string): vscode.TreeItem {
  ...
  return {
    label: /** vscode.TreeItemLabel**/ {
      label: key,
      highlights:
        key.length > 1 ? [[key.length - 2, key.length - 1]] : undefined,
    } as any,
    tooltip,
    collapsibleState:
