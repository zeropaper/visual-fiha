...
export type textLines = (lines: string[], opts?: {
  x?: number
  y?: number
  lineHeight?: number
  position?: string
...
export type mirror = (distance?: number, axis?: 'x' | 'y', img?: OffscreenCanvas) => void
...
export type grid = (rows: number, cols: number, func: (...args: any[]) => void) => void

export type centeredGrid = (opts: {
  cols?: number
  rows?: number
  dist?: number
